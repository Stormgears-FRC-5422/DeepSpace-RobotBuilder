package org.usfirst.frc5422.Minimec.subsystems.dsio;

public class ButtonIds {
    //TODO: Keep adding buttons
    // fix the JoyStickThings on the ButtonBoard

    public static final int JOYSTICK_PORT_DRIVING = 0;
    public static final int JOYSTICK_PORT_1 = 1;
    public static final int JOYSTICK_PORT_2 = 2;

    // **********
    //Button controller Port 2
    // **********
    public static final int ELEVATOR_REST_BUTTON_ID = 5;
    public static final int ELEVATOR_LEVEL_GROUND_BUTTON_ID = 6;
    public static final int ELEVATOR_LEVEL_1_BUTTON_ID = 7;
    public static final int ELEVATOR_LEVEL_2_BUTTON_ID = 8;
    public static final int ELEVATOR_LEVEL_3_BUTTON_ID = 9;

    public static final int WRIST_SWITCH_ID = 11; //Off = false
    public static final int INTAKE_BUTTON_ID = 10; //really now a button

    public static final int HATCH_STATUS_INTAKE_BUTTON_ID = 3;
    public static final int HATCH_STATUS_RELEASE_BUTTON_ID = 4;

    // **********
    //Button controller Port 1
    // **********
    public static final int CARGO_STATUS_INTAKE_BUTTON_ID = 1;
    public static final int CARGO_STATUS_RELEASE_BUTTON_ID = 2;

    public static final int ARM_PICKUP_POSITION = 3;
    public static final int ARM_REST_BUTTON_ID = 4;
    public static final int ARM_MOVE_90_BUTTON_ID = 5;
    public static final int ARM_MOVE_TO_135_BUTTON_ID = 6;



    public static final int BACKJACK_LEVEL_3 = 8;
    public static final int BACKJACK_LEVEL_2 = 9;
    public static final int RESET_CODE_BUTTON_ID = 10; //The switch with the red cap on it
    public static final int MOVE_BACKJACK = 7;


    //These are the buttons on the top. Going from left to right
//    public static final int MOVE_ARM = 3;
//    public static final int EXTRA_BUTTON_2 = 3;
//    public static final int EXTRA_BUTTON_3 = 3;
//    public static final int EXTRA_BUTTON_4 = 3;
//    public static final int EXTRA_BUTTON_5 = 3;

    public static final int XBOX_BUTTON_RIGHT_STICK = 10;
    public static final int XBOX_BUTTON_A = 1;
    public static final int XBOX_BUTTON_B = 2;
    public static final int XBOX_BUTTON_X = 3;
    public static final int XBOX_BUTTON_Y = 4;

}
