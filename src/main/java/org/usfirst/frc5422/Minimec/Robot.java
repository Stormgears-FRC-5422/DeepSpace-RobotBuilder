...
//        pixyVision = new PixyVision("vision"); // FIXME - read table name from config file?
    ...
    private boolean xboxController = true;
    @Override
    public void teleopPeriodic() {
        Scheduler.getInstance().run();
        if(xboxController){
            mecanum.driveCartesian(dsio.getJoystick().getRawAxis(0)*-1,(dsio.getJoystick().getRawAxis(3)-dsio.getJoystick().getRawAxis(2))*-1,dsio.getJoystick().getRawAxis(4));
        }
        else {
            mecanum.driveCartesian(dsio.getJoystick().getX(),dsio.getJoystick().getY(),dsio.getJoystick().getZ());
        }
    }
